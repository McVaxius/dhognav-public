...
internal sealed class ModuleLoader : IDisposable
{
    ...
    public void Load(IDalamudPluginInterface pluginInterface)
    {
        ...
        try
        {
            ...
            InitializeModule(new global::DhogNav.Plugin(), pluginInterface);
#else
            var folder = Path.Combine(pluginInterface.GetPluginConfigDirectory(), "tasks");
            var path = Path.Combine(folder, "DhogNav.Access.dll");
            if (!File.Exists(path))
            {
            ...
            var privateJsonPath = Path.Combine(folder, "DhogNav.json");
            XA.Access.PrivateManifest.Validate(bytes, File.Exists(privateJsonPath) ? XA.Access.PrivateManifest.ReadFile(privateJsonPath) : null, "DhogNav");
    ...
    private sealed class ModuleContext : AssemblyLoadContext
    {
        ...
        public ModuleContext() : base("DhogNav.Access." + Guid.NewGuid().ToString("N"), true) { }
