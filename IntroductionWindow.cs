...
internal sealed class IntroductionWindow : Window
{
    ...
    public IntroductionWindow(IDalamudPluginInterface pluginInterface, ITextureProvider textures)
        : base($"DhogNav {Version}###DhogNav.PublicShell.Introduction")
    {
    ...
    public override void Draw()
    {
        ...
        ImGui.BeginGroup();
        ImGui.TextColored(HeadingColor, "DhogNav");
        ...
        DrawPanel("Welcome to DhogNav",
            "This is the public introduction to DhogNav. It explains how to get the full plugin and install it with Aethertek Plugin Manager (APM).");
        ImGui.Spacing();
        ...
        DrawPanel("Install the full plugin with APM",
            "1. Copy the full-plugin ZIP URL from Discord (DhogNav.zip or DhogNav-v<version>.zip).\n" +
            ...
            "Keep APM enabled until the update finishes. The full plugin replaces this introduction and opens with /dnav.");
